...
function setupButtonEventListeners() {
    ...
    generateRaceWeatherBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            // Each missing slot is filled in a transaction, so a slot another driver
            // generated at the same moment is kept rather than overwritten.
            const results = await Promise.all(['weather1', 'weather2'].map(path =>
                firebase.database().ref(path).transaction(current =>
                    isWeatherSlotSet(current) ? undefined : generateRandomWeather()
                )
            ));

            updateWeatherSlot(0, results[0].snapshot.val());
            updateWeatherSlot(1, results[1].snapshot.val());
            if (weatherMessage) {
                weatherMessage.textContent = results.some(result => result.committed)
                    ? 'Race weathers generated! Please take a screenshot of the (probably terrible) outcome and share it with the group :)'
                    : 'Race weather has already been generated by another driver.';
            }
        } catch (error) {
...
function takeScreenshot() {
    html2canvas(document.body, {
        ...
        .then(blob => {
            const fileName = 'Race Weather.png';
            const file = typeof File === 'function' ? new File([blob], fileName, { type: 'image/png' }) : null;

            // Phones can share straight to the photo library or a chat; desktops download the file.
            if (file && navigator.canShare && navigator.canShare({ files: [file] })) {
                return navigator.share({ files: [file], title: 'Race Weather' }).catch(error => {
        ...
        .catch(error => {
            ...
            const weatherMessage = document.getElementById('weatherMessage');
            if (weatherMessage) weatherMessage.textContent = 'Unable to take a screenshot. Please use your device screenshot instead.';
        });
}
